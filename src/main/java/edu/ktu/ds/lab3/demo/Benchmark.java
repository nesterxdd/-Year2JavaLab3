...
//
//        Map<String,Car> carsMapOa = new HashMapOa<>(HashManager.HashType.DIVISION);
...
//    }
    ...
    @org.openjdk.jmh.annotations.Benchmark
    public void containsCarMapOa(FullMap fullmap){
        fullmap.ids.forEach(id -> fullmap.carsMapOa.contains(id));
    }

    @org.openjdk.jmh.annotations.Benchmark
    public void containsCarMap(FullMap fullmap){
        fullmap.ids.forEach(id -> fullmap.carsMap.containsKey(id));
    }
    ...
    public static void putHashMap(List<String> ids, List<Car> cars, HashMap<String, Car> carsMap) {
        for (int i = 0; i < cars.size(); i++) {
            carsMap.put(ids.get(i), cars.get(i));
        }
    }
