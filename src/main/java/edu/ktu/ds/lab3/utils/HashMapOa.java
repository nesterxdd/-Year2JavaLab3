...
public class HashMapOa<K, V> implements EvaluableMap<K, V> {
    ...
    public boolean replace(K key, V oldValue, V newValue) {
       if(key == null || oldValue == null || newValue == null){
           throw new IllegalArgumentException("Something is null in replace(K key, V oldValue, V newValue)");
       }
       int position = findPosition(key, false);
    ...
    public boolean containsValue(Object value) {
        ...
        for(Entry<K,V> pair : table){
            if(pair != null && pair != DELETED && pair.value.equals(value)){
                return true;
            }
        }
        return false;
