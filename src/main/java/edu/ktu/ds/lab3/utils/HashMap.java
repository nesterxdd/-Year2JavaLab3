...
public class HashMap<K, V> implements EvaluableMap<K, V> {
    ...
    public V remove(K key) {
        ...
        int sizeOfChainBefore = getChainSize(node);
        ...
        if(sizeOfChainBefore == maxChainSize){
            updateMaxChainSize();
        }
    ...
    //--------------------------------------------------------------------------------------------
    /**
     * Recompute the maximum chain size after a removal.
     *
     * @return The new maximum chain size.
     */
    private void updateMaxChainSize() {
        int newMaxChainSize = 0;
        for (Node<K, V> node : table) {

            int chainSize = getChainSize(node);
            newMaxChainSize = Math.max(newMaxChainSize, chainSize);

        }
        maxChainSize = newMaxChainSize;
    }

    /**
     * Returns the size of a chain starting from the given node.
     *
     * @param node The starting node of the chain.
     * @return The size of the chain.
     */
    private int getChainSize(Node<K, V> node) {
        int chainSize = 0;
        while (node != null) {
            chainSize++;
            node = node.next;
    ...
    public boolean replace(K key, V oldValue, V newValue) {
        ...
        {
            throw new IllegalArgumentException("Value is null in replace(K key, V oldValue, V newValue)");
        }
        int index = HashManager.hash(key.hashCode(), table.length, ht);
